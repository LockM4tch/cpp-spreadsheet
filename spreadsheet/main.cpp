...
namespace {
    ...
    void TestFormulaReferencedCells() {
        ...
        auto tricky = ParseFormula("A2 + A1 + A1 + A3 + A1 + A2 + A1");
        ASSERT_EQUAL(tricky->GetExpression(), "A2+A1+A1+A3+A1+A2+A1");
    ...
    void TestCellCircularReferences() {
        ...
        sheet->SetCell("B1"_pos, "=B2");
        sheet->SetCell("B2"_pos, "Ready");
...
int main() {
    ...
    RUN_TEST(tr, TestCahe);
 // написать тесты дл€ кэшей и создать саммехенизм кэшировани€
