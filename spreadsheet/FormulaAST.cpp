...
public:
    ...
    double Evaluate(const SheetInterface* sheet) const override {
        ...
        auto lhsValue = lhs_->Evaluate(sheet);
        auto rhsValue = rhs_->Evaluate(sheet);

        switch (type_){
            case (Type::Divide):
                if (0.0000001 > std::abs(rhsValue)) { 
                    throw FormulaError(ArithmeticError);
                }
                out = lhsValue / rhsValue;
                break;
            case (Type::Add):
                out = lhsValue + rhsValue;
                break;
...
public:
    ...
    double Evaluate(const SheetInterface* sheet ) const override {
        auto value = operand_->Evaluate(sheet);
        if (type_ == UnaryMinus) {
            return -(value);
        }
        else
...
public:
    ...
    double Evaluate(const SheetInterface* sheet) const override {
        ...
        const FormulaError::Category ValueError = FormulaError::Category::Value;
        ...
        if (std::holds_alternative<double>(val)) {
            ...
            try {
                size_t pos;
                std::string_view str = std::get<std::string>(val);
                double  val = std::stod(str.data(), &pos);
                if (pos != str.length()) {
                    throw FormulaError(ValueError);
                }
                    return val;
            }
